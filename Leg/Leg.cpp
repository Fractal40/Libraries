...
Leg::Leg(int coxaLen, int femurLen, int tibiaLen, int servoNrCoxa, int servoNrFemur, int servoNrTibia)
  ...
  void Leg::legOffset(int xOffset, int yOffset, int zOffset)
  {
    _xOffset = xOffset;
    _yOffset = yOffset;
    _zOffset = zOffset;
...
/*void legParabola() {
  ...
  for (int servoIndex=1; servoIndex<3; servoIndex++) {
      lastPulse[i] = -STEPHEIGHT * pow(paraCounter,2) + STEPHEIGHT * stepLen * paraCounter;
      lastPulse[i] = -STEPHEIGHT * pow(abs(lastPulse[i]-pulse[i]),2) + STEPHEIGHT * stepLen * paraCounter;
